// API endpoint para subir fotos de perfil de clientes a Cloudinary
...
async function validateClientSession(req: NextApiRequest): Promise<boolean> {
  ...
  try {
    ...
    const result = await sql`
      SELECT id FROM clients WHERE id = ${session.id} AND status = 'active' LIMIT 1
    `;
...
export default async function handler(
  ...
  // Validar autenticación de cliente
  const isAuthenticated = await validateClientSession(req);
  if (!isAuthenticated) {
  ...
  try {
    ...
    const result = await cloudinary.uploader.upload(file.filepath, {
      folder: 'e-bio-link/client-profiles', // Carpeta específica para clientes
      transformation: [
      ...
      public_id: `client-${Date.now()}`, // ID único
    });
